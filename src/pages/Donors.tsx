...
const Donors: React.FC = () => {
  ...
  const calculateTotalDonations = (donorId: string) => {
    return donations
      .filter(donation => donation.donor === donorId)
  ...
  const donorColumns: Column<Donor & { totalDonations: number }>[] = [
    { key: "name", label: "Name" },
    { key: "number", label: "Phone Number" },
    { key: "totalDonations", label: "Total Donations", render: (donor) => formatCurrency(donor.totalDonations) },
    { 
      key: "actions", 
      label: "Actions", 
      render: (donor) => (
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid gap-4 md:grid-cols-2">
        <CustomCard title="Donors List">
          <CustomTable data={donorsWithTotalDonations} columns={donorColumns} />
        </CustomCard>
        <CustomCard title="Add New Donor">
          <form className="flex flex-col gap-4">
            <FormInput
      ...
      {selectedDonor && (
        <CustomCard title={`Donor Details: ${selectedDonor.name}`}>
          <div className="mb-4">
            <p><strong>Phone Number:</strong> {selectedDonor.number}</p>
            <p><strong>Address:</strong> {selectedDonor.address}</p>
            <p><strong>Total Donations:</strong> {formatCurrency(selectedDonorTotalDonations)}</p>
          </div>
          <div>
            <h5 className="text-lg font-semibold mb-2">Donation History</h5>
            <CustomTable data={donorDonations} columns={donationColumns} />
